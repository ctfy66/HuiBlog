...
async function convertNotionPageToMarkdown(client, n2m, pageId) {
  try {
    ...
    // content 可能是字符串或对象 {parent: string}
    let markdownContent = "";
    if (typeof content === "string") {
      markdownContent = content;
    } else if (content && typeof content.parent === "string") {
      markdownContent = content.parent;
    }

    if (!markdownContent || markdownContent.trim() === "") {
      console.warn(
        `  Warning: Page ${pageId} has no content or unsupported block types`
      );
    }
...
function loadSyncMap() {
  try {
    ...
    console.warn(
      "Warning: Failed to load sync map, creating new one:",
      error.message
    );
  }
  ...
  try {
    ...
    if (fs.existsSync(publicPostsDir)) {
      ...
      const imagePattern = new RegExp(
        `^${pageId}-\\d+\\.(png|jpg|jpeg|gif|webp|svg)$`,
        "i"
      );
...
async function saveMarkdownFile(
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    try {
      ...
      console.warn(
        `  Warning: Failed to download image ${imageUrl}:`,
        error.message
      );
      // 保留原始URL
    }
...
async function main() {
  try {
    ...
    const notionPageIds = new Set(
      publishedPosts.map((post) => post.id.replace(/-/g, ""))
    );
    ...
    for (const post of publishedPosts) {
      try {
        ...
        // 转换内容
        const { title, date, content, images, lastEditedTime } =
          await convertNotionPageToMarkdown(client, n2m, pageId);
